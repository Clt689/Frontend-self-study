        ...
        /*
                // 초기 버스 위치 설정
                var markerPosition = new kakao.maps.LatLng(35.14272922274663, 126.93441741635835);
                ...
                // 버스 위치 업데이트 함수 (실제로는 서버에서 주기적으로 호출되어야 함)
                function updateBusLocation() {
                    // 새로운 버스 위치 정보를 가져온다.
                    // 새로운 위치 정보는 newMarkerPosition에 할당된다.
                    var newMarkerPosition = new kakao.maps.LatLng(35.141939023044976, 126.92780114915257);
        
                    // 기존 마커의 이미지를 업데이트
                    markerImage.src = '/02/그림2.svg';
        
                    // 마커의 위치를 새로운 위치로 업데이트
                    marker.setPosition(newMarkerPosition);
                }
        
                // 일정 간격으로 버스 위치 업데이트 함수 호출 (실제로는 서버에서 주기적으로 위치 정보 업데이트를 호출해야 함)
                setInterval(updateBusLocation, 1000); // 1초마다 업데이트, 원하는 시간으로 조절
        */
